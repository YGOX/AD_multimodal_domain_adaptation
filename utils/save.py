...
def saveModelandMetrics(modelObj, val_acc, val_loss, val_f1):
	# Save Prameters
	...
	savePickle(location=modelObj.expt_folder, file=file_names['output']['valid_bal_accuracy'], data=modelObj.valid_bal_accuracy)
	...
	file = open(os.path.join(modelObj.expt_folder,file_names['output']['best_val']), "w")
	file.write("acc:%0.6f,loss:%0.6f,f1:%0.6f"%(val_acc, val_loss, val_f1))
	file.close()
