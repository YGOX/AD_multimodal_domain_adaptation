...
img_loc = paths['data']['Raw_PET_location']
...
for idx, row in df.iterrows():
	if row['Phase'] != 'ADNI3':
		...
		if selected_files:
			dates_str = [re.split(r'[_]', fn)[-4] for fn in selected_files]
			file_dates = [datetime.strptime(d[:8], '%Y%m%d') for d in dates_str]
...
df_new.to_csv(os.path.join(paths['data']['Input_to_Training_Model'],
						   file_names['data']['PET_to_curr_label_mapping']))
