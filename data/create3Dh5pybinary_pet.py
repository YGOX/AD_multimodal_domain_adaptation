...
hdf5_path = os.path.join(paths['data']['Input_to_Training_Model'], file_names['data']['ad_nc_pet_hdf5_file'])
...
data_shape4 = (label_count['AD']+label_count['NL'], 1, 120,132,96)
...
for idx, row in curr_labels_df.iterrows():
    ...
    if label != 'MCI':
        ...
        img = np.swapaxes(img_nib.get_fdata(), 0, 3)
        ...
        df_new.to_csv(os.path.join(paths['data']['Input_to_Training_Model'],
                                   file_names['data']['pet_ad_nc_mapping']))
        id += 1
